...
bot.on('message.private', (e, context) => {
  if (context.user_id == setting.admin) {
    ...
    if (search) {
      ...
      replyMsg(
        context,
        `已封禁${search[1] == 'u' ? '用户' : '群组'}${search[1]}`
      )
      return
    }

    //明日方舟
    if (context.message == '--update-akhr')
      Akhr.updateData().then(() => replyMsg(context, '数据已更新'))
  }
})
...
bot
  .on('socket.connecting', (wsType, attempts) =>
    console.log(`${getTime()} 连接中[${wsType}]#${attempts}`)
  )
...
async function searchImg (context, customDB = -1) {
  ...
  for (let img of imgs) {
    if (hasCommand('get-url'))
      replyMsg(
        context,
        img.url.replace(/\/[0-9]+\//, '//').replace(/\?.*$/, '')
      )
    else {
      //获取缓存
      let hasCache = false
...
function doOCR (context) {
  ...
  for (let img of imgs) {
    ocr
      .default(img.url, lang)
      .then(ret => replyMsg(context, ret.join('\n')))
      .catch(e => {
  ...
  if (setting.akhr.enable) {
    ...
    for (let img of imgs) {
      ocr[setting.akhr.ocr](img.url, 'chs')
        .then(words => {
          // fix some ...
          if (setting.akhr.ocr == 'ocr.space')
            words = _.map(words, w => w.replace(/冫口了/g, '治疗'))
          replyMsg(
            context,
            `[CQ:image,file=base64://${Akhr.getResultImg(words)}]`
          )
        })
        .catch(e => {
